...
%        if skipRec == 1; continue; end  % to skip already analyzed records
    ...
    for iCond = 1:numConds
        ...
        if skipRec==1 && (length(dir([saveAnalysisDir '*FuncCon_' condName '*.fig'])) >= (doMean+doMedian))
    ...
    %    set(gca,'YDir','normal','TickDir','out','YTick',tickLoc,'YTickLabel',tickLabel)
        ...
        savefig(fig, ['meanFuncCon_' condName '_' num2str(lowFreq) '-' num2str(highFreq) 'Hz.fig'],'compact');
        saveas(fig, ['meanFuncCon_' condName '_' num2str(lowFreq) '-' num2str(highFreq) 'Hz.png']);
