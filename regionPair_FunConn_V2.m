function regionPair_FunConn_V2(skipRec, linORlog, lowFreq, highFreq, numFreqs, lfpFs, sessionID,...
...
for iPair = 1:numel(regionPairs)
    ...
    elseif length(dir([saveAnalysisDir '*.fig'])) >= 2*length(condID) %each condition generate a fig file
    ...
    for iCond = 1:numConds
        ...
        if skipRec ==1 && length(dir([saveAnalysisDir '*.fig'])) >= iCond;continue; end % skip already processed condition
        
        evtTime = evtTimes{iCondID};     
        twin    = twins{iCondID};
        baseTwin = baseTwins{iCondID};
        ...
        for iReg1Chn = 1:numChnReg1
            ...
            for iReg2Chn = 1:numChnReg2
                ...
                funcCon = is_functionalConnectivity_V2(xser,yser,regionXname,regionYname,condNames{iCondID},...
    ...
    cd(saveAnalysisDir)
    % save means
    save(['funcCon_avg_' condNames{iCondID} '.mat'],'tvec','fois','avgXSpec','avgYSpec','avgXNormed', 'avgYNormed','avgPLV','avgCoherency','avgImagZ','avgpsiNorm', '-v7.3');
    fprintf('\nDone saving mean');
    end
   
    % save all channel data
    save(['specAll_' condNames{iCondID} '.mat'],'tvec','foi','xSpecAll','ySpecAll','xSpecNormedAll','ySpecNormedAll','-v7.3');
    save(['plvAll_' condNames{iCondID} '.mat'],'plvAll','-v7.3');
    save(['coherencyAll_' condNames{iCondID} '.mat'],'coherencyAll','-v7.3');
    save(['psiAll_' condNames{iCondID} '.mat'],'psiAll','psiFreq','psiNormAll','-v7.3');
    try
    save(['GCAll_' condNames{iCondID} '.mat'],'tvecGC','GC_XtoY_All','GC_YtoX_All','avgGC_XtoY','avgGC_YtoX','-v7.3');
    catch
    end
    
    fprintf('\nDone saving channel data ============================================\n')
    ...
    save(['funcCon_median_' condNames{iCondID} '.mat'],'tvec','foi','avgXSpec','avgYSpec','avgXNormed', 'avgYNormed','avgPLV','avgCoherency','avgImagZ','avgpsiNorm', '-v7.3');
    try
    save(['GC_medain_' condNames{iCondID} '.mat'],'tvecGC','avgGC_XtoY','avgGC_YtoX','-v7.3');
    catch
    end
    fprintf('\nDone saving median ============================================\n')
    end
    ...
    %    set(gca,'YDir','normal','TickDir','out','YTick',tickLoc,'YTickLabel',tickLabel)
        ...
        savefig(fig, ['medianFuncCon_' condNames{iCondID} '_' num2str(lowFreq) '-' num2str(highFreq) 'Hz.fig'],'compact');
        saveas(fig, ['medianFuncCon_' condNames{iCondID} '_' num2str(lowFreq) '-' num2str(highFreq) 'Hz.png']);
        saveas(fig, [GroupAnalysisDir 'medianFC_' sessionID '_' regionXname '-' regionYname '_' condNames{iCondID} '.png']);        
        ...
        if doMean == 1
        load(['funcCon_avg_' condNames{iCondID} '.mat']);
        try
        load(['GCAll_' condNames{iCondID} '.mat'],'avgGC_XtoY','avgGC_YtoX');
        catch
        end
        % Plot
        screensize = get( groot, 'Screensize' );
    ...
    %    set(gca,'YDir','normal','TickDir','out','YTick',tickLoc,'YTickLabel',tickLabel)
        ...
        savefig(fig, ['meanFuncCon_' condNames{iCondID} '_' num2str(lowFreq) '-' num2str(highFreq) 'Hz.fig'],'compact');
        saveas(fig, ['meanFuncCon_' condNames{iCondID} '_' num2str(lowFreq) '-' num2str(highFreq) 'Hz.png']);
        saveas(fig, [GroupAnalysisDir 'meanFC_' sessionID '_' regionXname '-' regionYname '_' condNames{iCondID} '.png']);        
