function [r,t,p,hboot,CI,H,pH,line] = bendcorr(X,Y,fig_flag,beta)
...
%          AH 20210520: line: includes intercept and slope for fitted line 
%
...
else % not plot but still compute
    coef = pinv([X(:,1) ones(n,1)])*X(:,2);
    s = r / (std(X(:,1))/std(X(:,2)));
end
% AH 20210520: added line parameter for reproducing the line
line.slope = s;
line.intercept = coef(2);
% h = refline(s,coef(2)); set(h,'Color','r','LineWidth',4); % For plotting

if nargout > 3 % "~" also count as output
    line.CIslope = CIslope;
    line.CIintercept = CIintercept;
end
end
...
omega = W(m,:); % 20% cutoff of data furthest from median

%% Compute the correlation
% ------------------------
P = (X-M)./ repmat(omega,size(X,1),1); % each sample is expressed as how many 80% away from median
P(isnan(P)) = 0; P(isinf(P)) = 0; % correct if omega = 0
comb = [(1:size(X,2)/2)',((1:size(X,2)/2)+size(X,2)/2)']; % all pairs of columns (1-2, 1-3, 2-3, etc.)
r = NaN(size(comb,1),1); 
t = r; p = t;
...
for j = 1:size(comb,1)
    ...
    % column 1
    psi = P(:,comb(j,1)); 
    i1 = length(psi(psi<-1)); % outside -1 median away
    i2 = length(psi(psi>1));  % outside +1 median away
    sx = X(:,comb(j,1)); 
    sx(psi<(-1)) = 0; % set outlier to 0
    sx(psi>1) = 0;    % set outlier to 0
