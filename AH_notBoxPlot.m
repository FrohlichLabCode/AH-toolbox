...
%
% Also see: boxplot
% modified by AH @1/31/2020: change face alpha to 0.5, 
% add switch to markMean
...
function [h,statsOut]=myPlotter(X,Y)
    ...
    for k=1:length(X)
        ...
        h(k).data=plot(thisX, thisY, 'o', 'color', C,...
                       'markerfacecolor', C+(1-C)*0.65,'MarkerFaceAlpha',0.4);
    ...
    if strcmp(style,'line')
        for k=1:length(X)
            ...
            h(k).mu=plot(X(k),mu(k),'o','color','k',...
                'markerfacecolor','k',...
                'markersize',10); %AH: change color from r to k
            end
