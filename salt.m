...
%           AH: Should be the same length of the test period.
%
%   Output arguments:
...
%   AH: SALT is originally developed for inhibitory opto tagging (increase firing
%   latendy, so could be not as sensitive for activating opsin)
...
for k = 1:tno_test   % loop through trials
    cspt = spt_test(k,1:nmbn);   % current test window (entire test period)
    pki = find(cspt,1,'first');   % first spike in window (only 1st spike matters)
    if ~isempty(pki)
...
hlsi(:,next) = hst(1:end-1);   % latency histogram
nhlsi(:,next) = hlsi(:,next) / sum(hlsi(:,next));   % normalized latency histogram
% last column is the testing period
...
for k1 = 1:kn
    ...
    for k2 = k1+1:kn
        ...
        % jsd is an upper triangle matrix
    end
end
...
p_value = length(find(nullhypkld>=testkld)) / sno; % how many null latency is longer than test latency
