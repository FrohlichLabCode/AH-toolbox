function regionPair_FunConn_V2(skipRec, linORlog, lowFreq, highFreq, numFreqs, lfpFs,...
...
for iPair = 1:numel(regionPairs)
    ...
    cd(saveAnalysisDir)
    save(['funcCon_avg_' condNames{condID(iCond)} '.mat'],'avgXSpec','avgYSpec','avgXNormed', 'avgYNormed','avgPLV','avgCoherency','avgImagZ','avgpsiNorm', '-v7.3');

    % calculate median of all measures
    ...
    avgGC_XtoY   = squeeze(nanmedian(nanmedian( GC_XtoY_All ,1),2));
    avgGC_YtoX   = squeeze(nanmedian(nanmedian( GC_YtoX_All ,1),2));
    ...
    save(['specAll_' condNames{condID(iCond)} '.mat'],'tvec','foi','xSpecAll','ySpecAll','-v7.3');
    save(['plvAll_' condNames{condID(iCond)} '.mat'],'plvAll','-v7.3');
    save(['coherencyAll_' condNames{condID(iCond)} '.mat'],'coherencyAll','-v7.3');
    save(['psiAll_' condNames{condID(iCond)} '.mat'],'psiAll','psiFreq','psiNormAll','-v7.3');
    try
    save(['GCAll_' condNames{condID(iCond)} '.mat'],'tvecGC','GC_XtoY_All','GC_YtoX_All','avgGC_XtoY','avgGC_YtoX','-v7.3');
    catch
    end
    fprintf('\nDone saving ============================================\n')
    ...
    clear plvAll coherencyAll imagZAll psiNormAll GC_XtoY_All GC_YtoX_All 
    ...
    %    set(gca,'YDir','normal','TickDir','out','YTick',tickLoc,'YTickLabel',tickLabel)
        ...
        savefig(fig, ['avgFuncCon_' condNames{condID(iCond)} '_' num2str(lowFreq) '-' num2str(highFreq) 'Hz.fig'],'compact');
        saveas(fig, ['avgFuncCon_' condNames{condID(iCond)} '_' num2str(lowFreq) '-' num2str(highFreq) 'Hz.png']);
        close all
